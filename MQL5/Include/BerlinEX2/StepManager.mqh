...
class StepManager
{
    ...
    public:
        ...
        void stepCalculate(double box_size, double first_position_size, double input_size, int step_count)
        {
            ...
            for(int current=0; current<step_count; current++)
            {
                ...
                {
                    step[current].step              = current + 1;
                    step[current].lot               = first_position_size * 3;
                    step[current].net_lot           = step[current].lot - step[current-1].net_lot;
                }
                else
                {
                    step[current].step              = current + 1;
